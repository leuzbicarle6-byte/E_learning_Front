...
// Adapte ce chemin d'import selon l'emplacement réel de ton fichier data_excel.js
import { courExcelDetails } from "./data";
    ...
    <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden transition-all hover:border-indigo-400/40">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <span className="shrink-0 h-8 w-8 rounded-lg bg-linear-to-br from-emerald-500 to-teal-600 flex items-center justify-center">
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 space-y-3 border-t border-white/10">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <div className="rounded-lg bg-black/20 p-3">
              <p className="text-xs uppercase tracking-wide text-teal-400 mb-1">
                Exemple concret
              </p>
...
function TabContent({ tab }) {
  return (
    <div className="space-y-8">
      {tab.image && (
        <div className="rounded-2xl overflow-hidden border border-white/10 shadow-lg shadow-emerald-950/40">
          <img
            src={tab.image}
            alt={tab.label}
        ...
        {tab.sections.map((section, i) => (
          <div key={i}>
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-linear-to-r from-emerald-400 to-teal-400" />
              {section.name}
            </h3>
...
export default function ExcelDetail() {
  const course = courExcelDetails[0];
          ...
          {course.tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                activeTabId === tab.id
                  ? "bg-linear-to-r from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-900/40"
